                  ...
                  Net.Query.SendMsgQuery query $
                    Net.Query.ClientStQuerying
                      { Net.Query.recvMsgResult = \result -> do
                          ...
                          pure $
                            Net.Query.SendMsgRelease False $
                              pure $
                                Net.Query.SendMsgDone Nothing ()
                      }
            , Net.Query.recvMsgFailure = \failure -> do
                atomically $ putTMVar resultVar' (Left (toAcquiringFailure failure))
                pure $ Net.Query.SendMsgDone Nothing ()
            }
                  ...
                  Net.Query.SendMsgQuery query $
                    Net.Query.ClientStQuerying
                      { Net.Query.recvMsgResult = \result -> do
                          ...
                          pure $
                            Net.Query.SendMsgRelease shouldRelease $
                              pure $
                                Net.Query.SendMsgDone (guard shouldRelease *> Just leashId) ()
                      }
            , Net.Query.recvMsgFailure = \failure -> do
                atomically $ putTMVar resultVar' (Left (toAcquiringFailure failure))
                pure $ Net.Query.SendMsgDone (guard shouldRelease *> Just leashId) ()
            }
